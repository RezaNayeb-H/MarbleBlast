...
namespace zuma
{
	...
	class marble
	{
		public:
			...
			void render();
			void SetCurrentWayPoint(int n)
			{
			    mCurrentWaypoint = n;
			}
			void incrementCurrentWayPoint(int n = 1)
			{

			    mCurrentWaypoint += n;
			    if(mCurrentWaypoint < points.size() && mCurrentWaypoint >= 0)
			...
			vector2D pos = {SCREEN_WIDTH*2/3, SCREEN_HEIGHT*2/3};				// the position vector
			vector2D  vel;
	...
	int marble::handleFlyingState(int deltaT)
	{
		...
		for(int i = 0; i < path_marbles.size(); i++)
		{
			...
			if(CheckCircleCollision(pos, RADI+1, path_marbles[i]->pos, RADI+1))
			{
				SetCurrentWayPoint(path_marbles[i]->mCurrentWaypoint + DIST);
				currentState = in_path;
				connected = path_marbles[i]->connected;
				path_marbles.insert(path_marbles.begin() + i, this);
				for(int j = i+2; j < path_marbles.size(); j++)
				{
				    if(path_marbles[j]->connected == path_marbles[i]->connected)
                        path_marbles[j]->incrementCurrentWayPoint(DIST);
				}
				return i+1;
			}
		}
	...
	shooter::shooter(double x, double y)
	{
		...
		pos.y = y;
		ammo = new marble*[2];
		ammo[0] = new marble( &(mableTextures[rand() % MARBLE_MAX]), on_shooter, x, y - MARBLE_DISTANCE_FROM_BIRD );
		ammo[1] = new marble( &(mableTextures[rand() % MARBLE_MAX]), on_shooter, x, y - MARBLE_DISTANCE_FROM_BIRD );
		BPos = &pos;
		BAngle = &angle;
	}
	...
	void CreatePaths(std::vector<vector2D>* points)
	{
		...
		path->AddCurve({v2D(350.0f, 425.0f), v2D(350.0f, 650.0f), v2D(850.0f, 870.0f), v2D(750.0f, 600.0f)}, 300);
	...
	int marblePop(int k)
	{
		...
		{
			if(path_marbles[upper]->texture == path_marbles[upper+1]->texture && path_marbles[upper+1]->mCurrentWaypoint >= 0 && path_marbles[upper+1]->connected == path_marbles[k]->connected)
				upper++;
			else
				break;
		}
		...
		while(lower >= 1)
		{
			if(path_marbles[lower]->texture == path_marbles[lower-1]->texture && path_marbles[lower-1]->mCurrentWaypoint >= 0 && path_marbles[lower-1]->connected == path_marbles[k]->connected)
				lower--;
			else
				break;
		}
	...
	void SampleGameLoop()
	{
		...
		for(int i = 0; i<50; i++)
		{
			...
			path_marbles[i]->mCurrentWaypoint = i * DIST;
		}
		...
		while(!quit)
		{
			...
			{
				if(ii % 10 == 0) state = (*it)->handlePathState(deltaT);
                else
                    state = 0;
				if( state == -1 )
				{
				...
				if(it + 1 != path_marbles.end() && (*it)->mCurrentWaypoint + DIST >= (*(it+1))->mCurrentWaypoint)
					(*(it+1))->connected = (*it)->connected;
				(*it)->render();
			}
            ...
            if(jj++ % 300 == 0) {
            std::cout << "path_marbles:\n";
			for(int i=0; i < path_marbles.size(); i++)
            {
                std::cout << i << ": "<< path_marbles[i]->pos << "\n";
            }

            std::cout << "flying_marbles:\n";
			for(int i=0; i < flying_marbles.size(); i++)
            {
                std::cout << i << ": "<< flying_marbles[i]->pos << "\n";
            }
            }
		}
	}
